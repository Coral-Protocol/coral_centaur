...
# ── Coral server sidecar (started if CORAL_ENABLED=true) ────────────────────
if [ "${CORAL_ENABLED:-0}" = "1" ] && [ -f /opt/coral-server/coral-server.jar ]; then
    CORAL_AGENTS_DIR="$HOME_DIR/coral-agents"
    CORAL_CONFIG_DIR="$HOME_DIR/.coral-config"
    mkdir -p "$CORAL_CONFIG_DIR" "$HOME_DIR/.coral/logs" "$HOME_DIR/.coral/agents"

    # Symlink agent configs into ~/.coral/agents/ (default scan path)
    for agent_dir in "$CORAL_AGENTS_DIR"/*/; do
        agent_name=$(basename "$agent_dir")
        ln -sfn "$agent_dir" "$HOME_DIR/.coral/agents/$agent_name"
    done

    # Build coral-server config.toml
    cat > "$CORAL_CONFIG_DIR/config.toml" << CORAL_EOF
[auth]
keys = ["test"]
...
CORAL_EOF
    ...
    echo "Starting coral-server sidecar on port 5555..."
    JAVA_HOME=/opt/coral-java PATH="/opt/coral-java/bin:$PATH" \
        java -jar /opt/coral-server/coral-server.jar \
        --config "$CORAL_CONFIG_DIR/config.toml" \
        > "$HOME_DIR/.coral/logs/coral-server.log" 2>&1 &
    CORAL_PID=$!
    echo "coral-server started (PID=$CORAL_PID)"

    # Wait briefly for coral-server to start
    for _i in $(seq 1 15); do
        if curl -fsS --noproxy '*' --max-time 2 http://localhost:5555/ >/dev/null 2>&1; then
